...
eval m (Var s)
	| evalError m  == True = (m, (Num 0))
	| otherwise = (m, (toValueStr (Map.lookup s m)))
...
eval m (Assign s a) 
	...
	| otherwise = let dupla = eval m a
				in if (evalError (fst dupla)) then
					...
					(Map.insert s (snd dupla) (fst dupla), snd dupla)
...
eval m (Field a)
	| evalError m  == True = (m, (Num 0))
	| (toBool (snd (eval (fst (eval m a)) (Op2 Lt (Lit (snd (eval m a))) (Lit (Num 0)))))) == True = (Map.insert "-1" (Str "ERROR\n") (fst (eval (fst (eval m a)) (Op2 Lt (Lit (snd (eval m a))) (Lit 0)))), Num 0) -- chequeo variables negativa
